 ...
 */
export function parseSmsMessage(message: SmsForwarderMessage): ParsedSmsMessage {
    ...
    const contentMatch = message.message.match(/<br\s*\/?>\s*Message:\s*(.+?)(?:\s*\(\d{1,2}\/\d{1,2}\/\d{2}\s+\d{1,2}:\d{2}\s*[AP]M\)|$)/i);
    ...
    content = content.replace(/\s*\(\d{1,2}\/\d{1,2}\/\d{2}\s+\d{1,2}:\d{2}\s*[AP]M\)\s*$/i, '').trim();
